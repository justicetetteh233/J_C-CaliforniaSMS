...
# this is used to show all the fees a student is expected to pay based on his class or term
    ...
    def studentBalanceForTerm(self,term_id,student_id):
        return {"Total_Balance": self.studenttotalfeespayableforterm(term_id,student_id)[-1]["totaldebt"] - self.studentFeesPaidForTerm(term_id,student_id)[-1]["total_payments"]}
    ...
    def studentFinancialReport(self,term_id,student_id):
        responsedata =[
          ...
          {"TOTAL": self.studentFeesPaidForTerm(term_id,student_id)[-1]["total_payments"] + self.studentBalanceForTerm(term_id,student_id)["Total_Balance"]}
            ]
        ]
